  ...
  for (i = 0; i <= limit; i++) {
    ...
    if (var->isStaticArray) {
      ...
      if (curAddr && checkAllocAndInit(curAddr)) {
        ...
        VG_(printf)("    elts[%d]: old = %u ", i, sequenceEltsToOutput[i]);
        sequenceEltsToOutput[i] = ptrContents;
        VG_(printf)("new = %u\n", sequenceEltsToOutput[i]);
      }
      else {
                                  ...
                                  char overrideIsInitialized)
{
  ...
  VG_(printf)("    printDtraceBaseValueSequence()\n");
...
/* 	} */
  ...
  for (i = 0; i <= limit; i++) {
    ...
    VG_(printf)("  i: %d, loc: %u (overrideInit: %d)\n", i, loc, overrideIsInitialized);
