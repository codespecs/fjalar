...
void outputDaikonVar(DaikonVariable* var,
		     ...
		     unsigned long upperBound, // upper bound of the array to print
                     // An array of pointers to sequence elements to print out - the range
                     // of legal indices into this array go from 0 to upperBound
  ...
  if (layersBeforeBase > 0)
    {
      ...
      // If we are printing .dtrace, set ptrParam, upperBound, and
      // sequenceEltsToOutput for the derived variable that we want to
      // print out:
      if ((DTRACE_FILE == outputType) && !isDummy) {
        ...
        // If we have never outputted a sequence before and we plan
        // to derive a variable that is a sequence, then we must
        // initialize sequenceEltsToOutput to a new array of pointer
        // of stuff to output:
        if (!sequenceEltsToOutput) {
          // We are going to keep on dereferencing a single element:
          if (derefSingleElement) {
            // Don't do anything for now
          }
          // We need to create a new array of pointers for
          // sequenceEltsToOutput:
          else if (!isDummy) {
            unsigned int i;
            int spacing = getBytesBetweenElts(var);
            ...
            sequenceEltsToOutput = VG_(malloc)((upperBound + 1) * sizeof(*sequenceEltsToOutput));
            for (i = 0; i <= upperBound; i++) {
              sequenceEltsToOutput[i] = ptrParam + (i * spacing);
            }

            for (i = 0; i <= upperBound; i++) {
              VG_(printf)("  elts[%d]: %u\n", i, (Addr)sequenceEltsToOutput[i]);
              VG_(printf)("\n");
            }
          }
        }
      ...
      outputDaikonVar(var,
                      ...
                      (isDummy ? 0 : sequenceEltsToOutput),
      ...
      // Free sequenceEltsToOutput or risk leaking major amounts of memory!
      if (sequenceEltsToOutput) {
        VG_(free)(sequenceEltsToOutput);
      }
  ...
  // derived member variables:
      ...
      for (i = memberVars->first; i != 0; i = i->next)
	{
	  ...
	  // (I tried it in gdb and it seems to work, though.)
          curVarOffset = curVar->data_member_location;
	  ...
	  if ((D_DOUBLE == curVar->varType->declaredType) &&
	    ...
	    curVarOffset -= 4;
          ...
          curVarBasePtr = basePtrValue + curVarOffset;
	  ...
	  if (printAsSequence &&
	      ...
	      for (arrayIndex = 0; arrayIndex <= curVar->upperBounds[0]; arrayIndex++)
		{
		  ...
		  stringStackPush(fullNameStack, &fullNameStackSize, "]");
                  ...
                  // If an existing sequenceEltsToOutput already exists,
                  // allocate a new sequenceEltsToOutput and assign to it
                  // the values of each original element +
                  // (arrayIndex * getBytesBetweenElts(curVar)) + curVarOffset:

                  // TODO: I don't think this is quite right for array
                  // flattening, so I'll have to think more about it:
                  if (!isDummy && sequenceEltsToOutput) {
                    void** newSequenceEltsToOutput = VG_(malloc)((upperBound + 1) *
                                                                 sizeof(*sequenceEltsToOutput));
                    unsigned int ind;
                    for (ind = 0; ind <= upperBound; ind++) {
                      newSequenceEltsToOutput[ind] =
                        sequenceEltsToOutput[ind] +
                        (arrayIndex * getBytesBetweenElts(curVar)) +
                        curVarOffset;
                    }

                    // Important!  Reassign it back:
                    sequenceEltsToOutput = newSequenceEltsToOutput;
                  }
		  ...
		  outputDaikonVar(curVar,
				  ...
				  (isDummy ? 0 :
				   // Need to add an additional offset:
                                   //curVar->varType->byteSize)),
				   curVarBasePtr + (arrayIndex *
						    getBytesBetweenElts(curVar))),
				  0,
				  isDummy,
				  (isDummy ? 0 :
				   upperBound),
	  ...
	  // Print out a regular member variable without array flattening
	  else
	    {
	      ...
	      DPRINTF("-- %s -- override %d repPtrLevels %d isString %d... %c %c\n",
              ...
              // If an existing sequenceEltsToOutput already exists,
              // allocate a new sequenceEltsToOutput and assign to it
              // the values of each original element + curVarOffset:
              if (!isDummy && sequenceEltsToOutput) {
                void** newSequenceEltsToOutput = VG_(malloc)((upperBound + 1) *
                                                             sizeof(*sequenceEltsToOutput));
                unsigned int ind;
                for (ind = 0; ind <= upperBound; ind++) {
                  newSequenceEltsToOutput[ind] = sequenceEltsToOutput[ind] + curVarOffset;
                }
