                     ...
                     DaikonFunctionInfo* varFuncInfo, char isEnter) // These uniquely identify which program point we are printing (only relevant for DynComp purposes)
{
  ...
  // .decls
      ...
      else
	{
      ...
      // layer above base to denote pointer types (what about static
      // array types?  Should we use '[]'?
      ...
      if ((DTRACE_FILE == outputType) && !isDummy)
	{
      ...
      outputDaikonVar(var,
		      ((varOrigin == DERIVED_FLATTENED_ARRAY_VAR) ?
		       ...
		       DERIVED_VAR),
		      numDereferences + 1,
		      isAlreadyDaikonSequence, // dereferencing should not change isAlreadyDaikonSequence status
		      stopExpandingArrays,
		      stopDerivingMemberVars,
		      allowVarDumpToFile,
		      trace_vars_tree,
	  ...
	  else
	    {
	      ...
	      outputDaikonVar(curVar,
			      ...
			      0,
                              ...
                              (derefSingleElement ? 0 : stopExpandingArrays),
			      // Complex conditional:
