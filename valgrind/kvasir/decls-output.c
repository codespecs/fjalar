...
// which holds the 'START PC' ADDRESSES (NOT names) of the program points which we are
// interested in tracing.  This holds addresses and NOT names so that we can
// eliminate program points not to be traced at translation time, NOT run-time.
// (We can't do that if we hold names because C++ requires name demangling, which
//  can only happen at run-time as far as I can ascertain from the Valgrind interface)
UInt* prog_pts_tree = NULL;
...
// (Every line in trace_prog_ps_input_fp must start with the START PC
//  address of the program point in DECIMAL FORMAT, then followed
//  by a valid program point name -
  ...
  while (fgets(line, 200, trace_prog_pts_input_fp))
    {
      ...
      // Remember to dynamically allocate a chunk of memory for start PC
      UInt* pStartPC = VG_(calloc)(1, sizeof(*pStartPC));
      ...
      // The first (space-delimited) token must be an unsigned int
      // that represents the start PC of the function to be traced
      startPCStr = strtok(newString, " ");

      //      VG_(printf)("startPCStr: %s\n", startPCStr);

      // Convert to an unsigned long (Addr)
      *pStartPC = strtoul(startPCStr, NULL, 10);

      //      VG_(printf)("%u\n", *pStartPC);

      tsearch((void*)pStartPC, (void**)&prog_pts_tree, compareUInts);
...
void printOneFunctionDecl(DaikonFunctionInfo* funcPtr, char isEnter, char faux_decls)
{
  ...
  // because we want to get return values (??? What? This doesn't matter,
  // right?  Because we only care about the function's START PC address,
  // not its return value)
  if (prog_pt_dump_fp && !isEnter)
    {
      // Print out 'START PC' address first ...
      fprintf(prog_pt_dump_fp, "%u", funcPtr->startPC);
      // ... followed by the Daikon name
      fputs(" ", prog_pt_dump_fp);
