...
#define STRCASECMP(soname, fnname) \
   ...
   int VG_REPLACE_FUNCTION_EZU(20120,soname,fnname) \
          ( const char* s1, const char* s2 ) \
   { \
      extern int VG_(tolower)(int); \
      register unsigned char c1; \
      register unsigned char c2; \
      while (True) { \
         c1 = VG_(tolower)(*(unsigned char *)s1); \
         c2 = VG_(tolower)(*(unsigned char *)s2); \
         if (c1 != c2) break; \
         if (c1 == 0) break; \
         s1++; s2++; \
      } \
...
#define STRNCASECMP(soname, fnname) \
   ...
   { \
      ...
      while (True) { \
         ...
         if (*s2 == 0) return 1; \
         \
         if (VG_(tolower)(*(unsigned char*)s1) \
             < VG_(tolower)(*(unsigned char*)s2)) return -1; \
         if (VG_(tolower)(*(unsigned char*)s1) \
             > VG_(tolower)(*(unsigned char*)s2)) return 1; \
         \
         s1++; s2++; n++; \
...
#define STRCASESTR(soname, fnname) \
   ...
   { \
      ...
      \
      /* assert(nlen >= 1); */ \
      UChar n0 = VG_(tolower)(n[0]);                 \
      \
      while (1) { \
         UChar hh = VG_(tolower)(*h);    \
         if (hh == 0) return NULL; \
         if (hh != n0) { h++; continue; } \
         \
         UWord i; \
         for (i = 0; i < nlen; i++) { \
            if (VG_(tolower)(n[i]) != VG_(tolower)(h[i]))  \
               break; \
         } \
         /* assert(i >= 0 && i <= nlen); */ \
         if (i == nlen) \
