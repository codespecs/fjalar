...

#if 0  // causes compile problems and not needed for Fjalar
#include "sysdep.h"
#endif
#define PATH_MAX 1024  // all that Fjalar needs from sysdep.h
// also from sysdep.h
...
// 2022.07.06 sharkov63: added startswith macro
...
char * program_name = (char*) "fjalar";

// these variables cannot be static as they are shared with Fjalar's version of dwarf.c
...
int do_lto_syms;        // never true for Fjalar
...
bool slurp_rela_relocs (Filedata *, unsigned long, unsigned long, Elf_Internal_Rela **, unsigned long *);
Elf_Internal_Sym * get_32bit_elf_symbols (Filedata *, Elf_Internal_Shdr *, unsigned long *);
Elf_Internal_Sym * get_64bit_elf_symbols (Filedata *, Elf_Internal_Shdr *, unsigned long *);
...
// versions of error and warn needed by Fjalar
...
static const char *
bfd_vmatoa (const char *fmtch, bfd_vma value)  // Fjalar: const added to remove compile warning
{
  ...
  if (mvar == NULL)
    {
      ...
      mvar = VG_(malloc) ("readelf.c: get_data", (size_t) amt + 1);
  ...
  if (extra_padding && num_printed < width)
    {
      ...
      FJALAR_DPRINTF ("%-*s", width - num_printed, " ");
      num_printed = width;
    }

  VG_(free) ((void *) alloced_symbol);
  return num_printed;
}
		  ...
		  bool                is_dynsym)
{
  ...
  for (i = 0; i < rel_size; i++)
	...
	case EM_Z80:
      ...
      if (rtype == NULL)
	FJALAR_DPRINTF (_("unrecognized: %-7lx"), (unsigned long) type & 0xffffffff);
      else
	FJALAR_DPRINTF (do_wide ? "%-22s" : "%-17.17s", rtype);
      ...
      else if (symtab_index)
	{
	  if (symtab == NULL || symtab_index >= nsyms)
	    {
	      FJALAR_DPRINTF (_(" bad symbol index: %08lx in reloc\n"),
	      ...
	      if (ELF_ST_TYPE (psym->st_info) == STT_GNU_IFUNC)
		{
		  ...
		  if (version_string)
		    printf (sym_info == symbol_public ? "@@%s" : "@%s",
			    version_string);
		  FJALAR_DPRINTF ("()%-*s", (int)(len <= width ? (width + 1) - len : 1), " ");
		}
	      else
		{
		...
		FJALAR_DPRINTF (_("<string table index: %3lu>"), psym->st_name);
	      else if (psym->st_name >= strtablen)
                {
		  FJALAR_DPRINTF (_("<corrupt string table index: %3lu>"),
			 psym->st_name);
		  res = false;
		}
	      else
		{
		  ...
		  if (version_string)
		    FJALAR_DPRINTF (sym_info == symbol_public ? "@@%s" : "@%s",
			    version_string);
		}
	      ...
	      if (is_rela)
		{
		  ...
		  if ((bfd_signed_vma) off < 0)
		    FJALAR_DPRINTF (" - %" BFD_VMA_FMT "x", - off);
		  else
		    FJALAR_DPRINTF (" + %" BFD_VMA_FMT "x", off);
		}
	    }
      ...
      else if (is_rela)
	{
	  ...
	  FJALAR_DPRINTF ("%*c", is_32bit_elf ? 12 : 20, ' ');
	  if ((bfd_signed_vma) off < 0)
	    FJALAR_DPRINTF ("-%" BFD_VMA_FMT "x", - off);
	  else
	    FJALAR_DPRINTF ("%" BFD_VMA_FMT "x", off);
	  ...
	  if (dumpdata->dump_sects)
	    {
	      /* Copy current flag settings.  */
              VG_(memcpy) (new_dump_sects, dumpdata->dump_sects,
			   dumpdata->num_dump_sects * sizeof (* new_dump_sects));

	      VG_(free) (dumpdata->dump_sects);
	    }
...
Elf_Internal_Sym *  // no static for Fjalar
...
process_section_headers (Filedata * filedata)
{
  ...
  if (do_sections && !do_header)
	...
	FJALAR_DPRINTF (_("In linked file '%s': "), filedata->file_name);
      if (! filedata->is_separate || process_links)
        FJALAR_DPRINTF (ngettext ("There is %u section header, "
		      "starting at offset 0x%lx:\n",
		      "There are %u section headers, "
		      "starting at offset 0x%lx:\n",
...
#define CHECK_ENTSIZE(section, i, type)					\
  ...
  for (i = 0, section = filedata->section_headers;
       ...
       i++, section++)
    {
      const char * name = SECTION_NAME_PRINT (section);  // Fjalar: const added to remove compile warning
	      ...
	      || startswith (name, ".zdebug_")))
	{
	  ...
	  if (do_debugging
	      ...
	      || (do_debug_lines    && streq (name, "line"))  // Fjalar: use simpler version
			      ...
			      unsigned int                strtab_sz)
{
  ...
  printf (_("\nImage fixups for needed library #%d: %s - ident: %lx\n"),
	  (int) fixup->fixup_needed, lib_name, (long unsigned int) fixup->needed_ident);
  printf
...
process_ia64_vms_dynamic_relocs (Filedata * filedata)
{
  ...
  VG_(memset) (&fixup, 0, sizeof (fixup));
  VG_(memset) (&imgrela, 0, sizeof (imgrela));
  ...
  if (do_using_dynamic)
	  ...
	  if (rel_size)
	    {
	      if (filedata->is_separate)
		FJALAR_DPRINTF
		  (_("\nIn linked file '%s' section '%s' at offset 0x%lx contains %lu bytes:\n"),
		   filedata->file_name, name, rel_offset, rel_size);
	      else
                FJALAR_DPRINTF
		  (_("\n'%s' relocation section at offset 0x%lx contains %lu bytes:\n"),
	   ...
	   i++, section++)
	{
	  ...
	  if (rel_size)
		...
		FJALAR_DPRINTF ("%u", section->sh_name);
	      else
		FJALAR_DPRINTF ("'%s'", printable_section_name (filedata, section));

	      num_rela = rel_size / section->sh_entsize;
	      FJALAR_DPRINTF (ngettext (" at offset 0x%lx contains %lu entry:\n",
...
#if 0 // do_unwind is never true for Fjalar
...
#endif // do_unwind is never true for Fjalar
...
#endif // do_version is never true for Fjalar
...
get_symbol_binding (Filedata * filedata, unsigned int binding)
{
  ...
  switch (binding)
	...
	snprintf (buff, sizeof (buff), _("<processor specific>: %u"),
		  binding);
      else if (binding >= STB_LOOS && binding <= STB_HIOS)
	{
	  if (binding == STB_GNU_UNIQUE
	    ...
	    return "UNIQUE";
	  snprintf (buff, sizeof (buff), _("<OS specific>: %u"), binding);
	}
      else
	snprintf (buff, sizeof (buff), _("<unknown>: %u"), binding);
      return buff;
    }
}
...
get_symbol_type (Filedata * filedata, unsigned int type)
{
  ...
  switch (type)
      ...
      if (type >= STT_LOPROC && type <= STT_HIPROC)
	{
	  ...
	  snprintf (buff, sizeof (buff), _("<processor specific>: %u"), type);
	}
      else if (type >= STT_LOOS && type <= STT_HIOS)
	{
	  ...
	  snprintf (buff, sizeof (buff), _("<OS specific>: %u"), type);
	}
      else
	snprintf (buff, sizeof (buff), _("<unknown>: %u"), type);
      return buff;
    }
}
...
get_symbol_index_type (Filedata * filedata, unsigned int type)
{
  ...
  switch (type)
	...
	sprintf (buff, _("bad section index[%3u]"), type);
      else
...
// START OF CODE ADDED FOR FJALAR
  ...
  if (((STT_OBJECT == ELF_ST_TYPE (psym->st_info)) ||
    ...
    if (STT_OBJECT == ELF_ST_TYPE (psym->st_info)) {
      insertIntoVariableSymbolTable(symbol_name, (void *)(ptrdiff_t)psym->st_value);
    }
    else if (STT_FUNC == ELF_ST_TYPE (psym->st_info)) {
      insertIntoFunctionSymbolTable(symbol_name, (void *)(ptrdiff_t)psym->st_value);
    }
  }
...
// END FJALAR CODE
  ...
  if (! do_wide && version_string != NULL)
    {
      ...
      len_avail -= 1 + VG_(strlen) (version_string);
  ...
  if (version_string)
    {
      if (sym_info == symbol_undefined)
	FJALAR_DPRINTF ("@%s (%d)", version_string, vna_other);
      else
	FJALAR_DPRINTF (sym_info == symbol_hidden ? "@%s" : "@@%s",
		version_string);
    }
...
// END FJALAR CODE
	  ...
	  if (is_32bit_elf)
	    FJALAR_DPRINTF (_("   Num:    Value  Size Type    Bind   Vis      Ndx Name\n"));
	  else
	    FJALAR_DPRINTF (_("   Num:    Value          Size Type    Bind   Vis      Ndx Name\n"));
  ...
  if (do_histogram && filedata->buckets != NULL)
    {
      ...
      lengths = (unsigned long *) VG_(calloc) ("readelf.c: process_symbol_table.1", filedata->nbuckets,
					  sizeof (*lengths));
      if (lengths == NULL)
	{
      ...
      VG_(memset) (visited, 0, filedata->nchains);
  ...
  VG_(free) (filedata->buckets);
  filedata->buckets = NULL;
  filedata->nbuckets = 0;
  VG_(free) (filedata->chains);
  filedata->chains = NULL;
  ...
  if (do_histogram && filedata->gnubuckets != NULL)
    {
      ...
      lengths = (unsigned long *) VG_(calloc) ("readelf.c: process_symbol_table.3", filedata->ngnubuckets,
					  sizeof (*lengths));
      if (lengths == NULL)
	{
	...
	if (filedata->gnubuckets[hn] != 0)
      ...
      counts = (unsigned long *) VG_(calloc) ("readelf.c: process_symbol_table.4", maxlength + 1, sizeof (*counts));
      if (counts == NULL)
	{
	  VG_(free) (lengths);
  ...
  VG_(free) (filedata->gnubuckets);
  filedata->gnubuckets = NULL;
  filedata->ngnubuckets = 0;
  VG_(free) (filedata->gnuchains);
  filedata->gnuchains = NULL;
  filedata->ngnuchains = 0;
  VG_(free) (filedata->mipsxlat);
  filedata->mipsxlat = NULL;
  return true;
...
process_syminfo (Filedata * filedata)
{
  ...
  if (filedata->is_separate)
    FJALAR_DPRINTF (ngettext ("\nIn linked file '%s: the dynamic info segment at offset 0x%lx contains %u entry:\n",
		      "\nIn linked file '%s: the dynamic info segment at offset 0x%lx contains %u entries:\n",
		      filedata->dynamic_syminfo_nent),
	    filedata->file_name,
	    filedata->dynamic_syminfo_offset,
	    filedata->dynamic_syminfo_nent);
  ...
  for (i = 0; i < filedata->dynamic_syminfo_nent; ++i)
	...
	FJALAR_DPRINTF (_("<corrupt index>"));
	...
	FJALAR_DPRINTF (_("<corrupt: %19lu>"), filedata->dynamic_symbols[i].st_name);
      putchar (' ');
	...
	default:
	  if (filedata->dynamic_syminfo[i].si_boundto > 0
	    ...
	    FJALAR_DPRINTF ("%-10d ", filedata->dynamic_syminfo[i].si_boundto);
	  break;
	}
			     ...
			     void *                           data)
{
  ...
  if (section->start == NULL)
    ...
    {
      // unsigned char *start = section->start; // unused in Fjalar
      dwarf_size_type size = sec->sh_size;
      // dwarf_size_type uncompressed_size = 0; // unused in Fjalar

#if 0 // Not needed for Fjalar??
...
{
  const char * name = SECTION_NAME_VALID (section) ? SECTION_NAME (section) : "";  // Fjalar: const added to remove warning
  const char * print_name = printable_section_name (filedata, section);
  bfd_size_type length;
  bool result = true; // Fjalar (observation): not always set correctly, but caller ignores
  int i;
...
// START OF CODE ADDED FOR FJALAR
  ...
  for (i = 0; i < max; i++)
	...
	if (load_specific_debug_section ((enum dwarf_section_display_enum) i,
                                         section, filedata))
          debug_displays[i].display (sec, filedata);
        break;
      }
...
#if 0 // do_arch is never true for Fjalar
...
{
  VG_(free) (filedata->program_interpreter);
  VG_(free) (filedata->program_headers);
  VG_(free) (filedata->section_headers);
  VG_(free) (filedata->string_table);
  VG_(free) (filedata->dump.dump_sects);
  VG_(free) (filedata->dynamic_strings);
  VG_(free) (filedata->dynamic_symbols);
  VG_(free) (filedata->dynamic_syminfo);
  VG_(free) (filedata->dynamic_section);

  while (filedata->symtab_shndx_list != NULL)
    {
      elf_section_list *next = filedata->symtab_shndx_list->next;
      VG_(free) (filedata->symtab_shndx_list);
      filedata->symtab_shndx_list = next;
    }

  VG_(free) (filedata->section_headers_groups);
  ...
  if (filedata->section_groups)
      ...
      for (i = 0; i < filedata->group_count; i++)
	{
      ...
      VG_(free) (filedata->section_groups);
    }
  VG_(memset) (&filedata->section_headers, 0,
...
fjalar_process_file (const char * file_name)
{
  ...
  filedata = VG_(calloc) ("readelf.c: fjalar_process_file.1", 1, sizeof * filedata);
  if (filedata == NULL)
    {
  ...
  filedata->file_size = (bfd_size_type) statbuf.size;
...
//  Fjalar does not need the gnu liblist info (do_arch always = 0).
//if (! process_gnu_liblist (filedata))
//  res = FALSE;

//  Fjalar does not need the arch specific info (do_arch always = 0).
...
// 2022.07.06 sharkov63: changed return type from bfd_boolean to int
int
process_elf_binary_data (const HChar* filename)
{
  ...
  if (cmdline.dump_sects != NULL) {
    VG_(free) (cmdline.dump_sects);
    cmdline.dump_sects = NULL;
  }
