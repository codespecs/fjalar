...
bfd_size_type current_file_size;
...
//static unsigned int  num_cmdline_dump_sects = 0;  // commented out for Fjalar to remove unused warning
...
process_section_contents (FILE * file)
{
  ...
  for (i = 0, section = section_headers;
       ...
       i++, section++)
    {
      // FJALAR_DPRINTF ("check section: %d, %s\n", i, printable_section_name (section));  // debugging code
#ifdef SUPPORT_DISASSEMBLY
...
fjalar_process_file (const char * file_name)
{
  ...
  SysRes res = VG_(stat) (file_name, &statbuf);
  if (sr_isError(res))
    {
      if (errno == 2) // ENOENT
  ...
  current_file_size = (bfd_size_type) statbuf.size;
...
process_object (char * file_name, FILE * file)
{
  ...
  if (num_cmdline_dump_sects > 0)
	...
	request_dump_bynumber (num_cmdline_dump_sects, 0);
      ...
      tl_assert (num_dump_sects >= num_cmdline_dump_sects);
