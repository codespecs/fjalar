	  ...
	  const char * reason)
{
  ...
  if (mvar == NULL)
	...
	mvar = VG_(malloc) ("readelf.c: get_data", size * nmemb + 1);
...
#define CHECK_ENTSIZE(section, i, type)					\
  ...
  for (i = 0, section = section_headers;
       ...
       i++, section++)
    {
      const char * name = SECTION_NAME (section);  // const added to remove compile warning
                   ...
                   || const_strneq (name, ".zdebug_")))
	{
	  ...
	  if (do_debugging
	      ...
	      || (do_debug_lines    && streq (name, "line"))  // don't know why original didn't use this simpler version
