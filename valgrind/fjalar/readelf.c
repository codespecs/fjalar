...
find_section (Filedata * filedata, const char * name)
{
  ...
  tl_assert (filedata->section_headers != NULL);
  ...
  for (i = 0; i < rel_size; i++)
      ...
      else if (symtab_index)
	{
	  if (symtab == NULL || symtab_index >= nsyms)
	    {
              FJALAR_DPRINTF (_(" bad symbol index: %08lx"), (unsigned long) symtab_index);
              res = FALSE;
	    }
          else
	    {
...
process_section_headers (Filedata * filedata)
{
  ...
  if (do_sections && !do_header)
    FJALAR_DPRINTF (ngettext ("There is %d section header, "
	  ...
	  if ((section->sh_flags & SHF_COMPRESSED) != 0)
	    {
		 ...
		 header + 12 bytes for compressed data header.  */
	      ...
	      tl_assert (sizeof (buf) >= sizeof (Elf64_External_Chdr));
...
process_syminfo (Filedata * filedata ATTRIBUTE_UNUSED)
{
  ...
  for (i = 0; i < dynamic_syminfo_nent; ++i)
	...
	FJALAR_DPRINTF (_("<corrupt: %19ld>"), dynamic_symbols[i].st_name);
      putchar (' ');
      ...
      else
	{
	  ...
	  while (section_len > 0)
	      ...
	      while (attr_len > 0 && p < contents + sect->sh_size)
		{
		  ...
		  tl_assert (end <= contents + sect->sh_size);
		  p += 4;
