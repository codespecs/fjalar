...
// static int decompress_dumps; // not used in Fjalar
...
char *dump_ctf_symtab_name;
char *dump_ctf_strtab_name;
#endif
...
static const char *
bfd_vmatoa (const char *fmtch, bfd_vma value)
{
...
#define CHECK_ENTSIZE(section, i, type)					\
  ...
  for (i = 0, section = filedata->section_headers;
       ...
       i++, section++)
    {
      const char * name = SECTION_NAME (section);  // Fjalar: const added to remove compile warning
...
{
  const char * name = SECTION_NAME (section);  // Fjalar: const added to remove warning
