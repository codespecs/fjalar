  ...
  for (i = 0; i < rel_size; i++)
      ...
      else if (symtab_index)
	{
	  if (symtab == NULL || symtab_index >= nsyms)
	    {
              FJALAR_DPRINTF (_(" bad symbol index: %08lx in reloc"), (unsigned long) symtab_index);
              res = FALSE;
	    }
	  else
	    {
		...
		FJALAR_DPRINTF (_("<string table index: %3lu>"), psym->st_name);
                ...
                {
		  FJALAR_DPRINTF (_("<corrupt string table index: %3lu>"), psym->st_name);
                  res = FALSE;
                }
	      else
		{
	  ...
	  if (filedata->dump_sects)
	    {
	      /* Copy current flag settings.  */
              VG_(memcpy) (new_dump_sects, filedata->dump_sects,
			   filedata->num_dump_sects * sizeof (* new_dump_sects));

	      VG_(free) (filedata->dump_sects);
	    }
...
print_dynamic_symbol (Filedata * filedata, bfd_vma si, unsigned long hn)
{
  ...
  if (VALID_DYNAMIC_NAME (psym->st_name))
    ...
    printf (_(" <corrupt: %14lu>"), psym->st_name);
  putchar ('\n');
}
  ...
  if (do_histogram && gnubuckets != NULL)
    {
      ...
      lengths = (unsigned long *) VG_(calloc) ("readelf.c: process_symbol_table.3", ngnubuckets, sizeof (*lengths));
      if (lengths == NULL)
	{
...
process_syminfo (Filedata * filedata ATTRIBUTE_UNUSED)
{
  ...
  for (i = 0; i < dynamic_syminfo_nent; ++i)
	...
	FJALAR_DPRINTF (_("<corrupt: %19lu>"), dynamic_symbols[i].st_name);
      putchar (' ');
...
//  res = FALSE;
  ...
  if (section_headers_groups)
    {
      VG_(free) (section_headers_groups);
